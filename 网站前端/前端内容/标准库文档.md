...
2. String原型链（继承给所有实例）：
    ...
    3. 跟HTML无关的方法：
        ...
        15. `String.prototype.padStart(目标长度数字[, 填充字符串])`
            ...
            1. 描述：字符串开头填充指定的填充字符串（不够则循环重复，太长则截断，默认为`' '`），直到长度达到目标长度数字（若目标长度数字小于等于原字符串长度，则不改变原字符串）。
            2. 返回：修改后的字符串。
        16. `String.prototype.padEnd(目标长度数字[, 填充字符串])`

            1. 描述：字符串尾部填充指定的填充字符串（不够则循环重复，太长则截断，默认为`' '`），直到长度达到目标长度数字（若目标长度数字小于等于原字符串长度，则不改变原字符串）。
            2. 返回：修改后的字符串。
...
JavaScript采用IEEE 754标准，数值存储为64位双精度格式，数值精度最多可以达到53个二进制位（1个隐藏位与52个有效位）。如果数值的精度超过这个限度，第54位及后面的位就会被丢弃。
...
2. Function原型链（继承给所有实例）：
    ...
    >
    >e.g. `arguments`不是~~Array~~类型所以无法调用原型链上的~~Array.prototype.slice~~，但可通过`Array.prototype.slice.call(arguments)`调用。
    ></details>
...
1. Promise对象
    ...
    - 特性
        ...
        2. Promise对象失败（返回失败状态或函数体内抛出错误）却没有被`try-catch`或`then/catch`处理，并且不在`await`调用的方法体内进行，则会触发`window`的`unhandledrejection`事件（错误不向上抛出、不交给`window`的`error`处理）。
        3. 以下方法均返回Promise对象，都可以继续链式调用`then`、`catch`、`finally`（解决回调地狱）。

    1. Promise构造函数：

        1. `new Promise((resolve, reject) => {/* resolve(参数); reject(参数) */}) // 第一参数必填且必须是方法`
...
2. 可迭代对象
    ...
    原生的可迭代对象：`Array`、`TypedArray`、`Map`、`Set`、`arguments`、`NodeList`、`String`、`迭代器`、`生成器`
...
4. 生成器（generators）
    ...
    2. 执行步骤：
        ...
        2. 每次调用生成器对象的`next`，函数解冻并运行至下一个`yield`或`return`，再次冻结。
            ...
            2. `yield`表达式本身默认返回值为`undefined`。`next`方法可以带一个参数，该参数就会被当作上一个`yield`表达式的返回值。
...
5. `async-await`（ES8）
    ...
    a() // async方法返回的是Promise对象；若方法体内抛出错误则状态失败，否则状态完成（包括默认返回的undefined）；返回值或抛出异常值为向下传递的值
    ```
    </details>
6. 字符串的扩展
...
7. 解构赋值（destructuring assignment）
    ...
    2. 对象的解构（ES9）：
        ...
        1. 可以嵌套任意深度的对象。
            ...
            >```
            ></details>
        2. 解构内部支持：**默认参数**、**剩余参数**（剩余参数仅解构自身（不在原型链上）的属性，且剩余参数仅能作用于变量名`...变量`、不能再嵌套解构）。
    3. 基本数据类型的解构：
    ...
    4. 形参的解构：
        ...
        ```javascript
        // 写法一
        function m1({x = 0, y = 0} = {}) { // 若传入的是undefined或空，则解构：{x = 0, y = 0} = {}；否则解构：{x = 0, y = 0} = 传入的值
          return [x, y];
        }

        // 写法二
        function m2({x, y} = { x: 0, y: 0 }) { // 若传入的是undefined或空，则解构：{x, y} = { x: 0, y: 0 }；否则解构：{x, y} = 传入的值
          return [x, y];
        }
...
8. 默认参数（default parameters）
    ...
    3. 调用函数时，当且仅当传参为`undefined`时才使用默认参数（不传参等效于传参为`undefined`）。
    4. 支持解构内部使用默认参数。
...
10. 展开元素（spread operator）

    把`可迭代对象`或`对象`，的值或变量，的自身（不在原型链上）的属性，全部拆分出使用。
...
12. `Symbol`
    ...
    新增的基本数据类型。可用作属性键，能避免命名冲突的风险（创建一个`Symbol`类型的值与其他任何值不会相等）。
...
13. `Set`、`Map`
    ...
    1. `Set`
        ...
        - 方法、属性：
            ...
            `new Set([可迭代对象])`、`Set.prototype.size`、`Set.prototype.has(值)`、`Set.prototype.add(值)`、`Set.prototype.delete(值)`、`Set.prototype.clear()`、`Set.prototype.forEach(回调函数(当前值, 索引也是当前值, Set整体)[, this替代])`、`Set.prototype.keys()`、`Set.prototype.values()`、`Set.prototype.entries()`
    2. `Map`

        类似对象的hash结构，键-值的`键`可以是任何值（对象的键只能是字符串）。
        ...
        - 方法、属性：
            ...
            `new Map([可迭代对象，成员为键-值的数组])`、`Map.prototype.size`、`Map.prototype.has(键)`、`Map.prototype.get(键)`、`Map.prototype.set(键, 值)`、`Map.prototype.delete(键)`、`Map.prototype.clear()`、`Map.prototype.forEach(回调函数(当前值, 当前键, Map整体)[, this替代])`、`Map.prototype.keys()`、`Map.prototype.values()`、`Map.prototype.entries()`
    3. `WeakSet`

        与`Set`类似（不重复的值的集合），但仅支持：`new WeakSet([可迭代对象])`、`Set.prototype.has(值)`、`Set.prototype.add(值)`、`Set.prototype.delete(值)`。
    ...
    4. `WeakMap`
        ...
        与`Map`类似（键可以是任何值），但仅支持：`new WeakMap([可迭代对象])`、`WeakMap.prototype.has(键)`、`WeakMap.prototype.get(键)`、`WeakMap.prototype.set(键, 值)`、`WeakMap.prototype.delete(键)`。
...
17. `Proxy`
    ...
    `get`、`set`、`has`、`ownKeys`、`getOwnPropertyDescriptor`、`defineProperty`、`preventExtensions`、`getPrototypeOf`、`isExtensible`、`setPrototypeOf`、`apply`、`construct`
18. `Reflect`

    操作对象的默认行为。
    
    与`Proxy`的handler方法一一对应的静态函数。
19. 计算对象的属性名或方法名
...
22. `class-extends`
    ...
    可以继承任何有`prototype`属性的函数（除了`Function.prototype`函数）和`null`。
23. `super`
